...
import AddAndUpateProject from "../Project/AddAndUpateProject.vue";
export default {
  props: ["ProjectProps"],
...
<template>
  ...
  >
    <h3 class="text-xl font-semibold py-3 text-truncate">
      {{
        $t("projectManagement.projectName") + ": " + ProjectProps.project.name
      }}
    </h3>
    <p class="text-blue-grey-darken-1 py-5 h-100 text-body-2 text-truncate">
      {{
        ...
        ": " +
        ProjectProps.project.description
      }}
    </p>
    ...
    <p class="text-blue-grey-darken-3 mb-3">
      {{ $t("Invitation.senderEmail") + ": " + CorrcetDate
