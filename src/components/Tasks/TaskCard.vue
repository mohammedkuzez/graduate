...
<template>
  ...
  >
    ...
    <div class="d-flex justify-space-between py-3">
      ...
      <p class="text-blue-grey-darken-3 mb-3">
        <v-icon icon="mdi-calendar-clock-outline"></v-icon>{{ EndDate }}
      </p>
    </div>
    <v-card-actions class="justify-center">
      <v-btn
        ...
        v-if="activeOwner"
        class="no-drag bg-warning w-50"
        >{{ $t("ProjectTasks.edit") }}</v-btn
      >
      <v-btn
        @touchstart.stop
        @mousedown.stop
        ...
        v-if="activeOwner"
        class="no-drag bg-red-accent-4 w-50"
        >{{ $t("ProjectTasks.delete") }}</v-btn
      >
    </v-card-actions>
    <Delete
