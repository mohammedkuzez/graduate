  ...
  methods: {
    ...
    async GetData() {
      ...
      if (this.GetTasksProject && this.GetTasksProject.length > 0) {
        ...
        this.board[1] = this.GetTasksProject.filter(
          (Task) => Task.status.name === "To Do"
        );
        this.board[2] = this.GetTasksProject.filter(
          (Task) => Task.status.name === "In Progress"
        );
        this.board[3] = this.GetTasksProject.filter(
          (Task) => Task.status.name === "Review"
        );
  ...
  <v-container fluid class="pa-0">
    <v-row class="flex-wrap">
      <v-col
        ...
        xl="2"
      >
        <v-card class="mx-2 pa-3 bg-success" elevation="3" rounded="lg">
